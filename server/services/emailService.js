 ...
 *
 * Recipients: Sairam (sairam@cittaa.in) and Abhijay (abhijay@cittaa.in)
...
const TEAM = ['sairam@cittaa.in', 'abhijay@cittaa.in'];
...
async function sendNewLeadEmail(lead) {
  const typeLabel = lead.type === 'school' ? '🏫 School' : '🏢 Corporate';
  const html = baseTemplate(
    ...
    <div style="background:#f9f7fc;border-left:4px solid ${PURPLE};border-radius:0 8px 8px 0;padding:20px;margin-bottom:24px;">
      <div style="font-size:18px;font-weight:700;color:${INK};margin-bottom:4px;">${lead.org_name}</div>
      <div style="margin-bottom:12px;">${badge(typeLabel)} ${badge(lead.city || '', TEAL)}</div>
      <table cellpadding="0" cellspacing="0">
        ...
        ${infoRow('Size', lead.employees_or_students ? `${lead.employees_or_students.toLocaleString('en-IN')} ${lead.type === 'school' ? 'students' : 'employees'}` : null)}
  ...
  await sendEmail({ to: TEAM, subject: `🎯 New Lead: ${lead.org_name} · ${lead.city || 'India'}`, html });
}
  ...
  const leadRows = topLeads.map((l) => `
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #ede8f4;">
        ...
        <div style="font-size:12px;color:#888;margin-top:2px;">${l.city || ''} · ${l.type === 'school' ? '🏫' : '🏢'} ${l.type}</div>
        ${l.ai_reasoning ? `<div style="font-size:12px;color:#666;margin-top:4px;font-style:italic;">${l.ai_reasoning}</div>` : ''}
      </td>
      <td style="padding:10px 0;border-bottom:1px solid #ede8f4;text-align:right;vertical-align:top;">
        <span style="display:inline-block;background:${l.ai_score >= 70 ? '#22c55e' : l.ai_score >= 45 ? '#f59e0b' : '#ef4444'}20;color:${l.ai_score >= 70 ? '#16a34a' : l.ai_score >= 45 ? '#d97706' : '#dc2626'};font-size:12px;font-weight:700;padding:3px 10px;border-radius:20px;">${l.ai_score}/100</span>
...
async function sendLeadApprovedEmail(lead, approvedBy) {
  const approverName = approvedBy === 'S' ? 'Sairam' : approvedBy === 'A' ? 'Abhijay' : approvedBy;
  const html = baseTemplate(
    `Lead Approved: ${lead.org_name}`,
    `
    ${sectionTitle('Lead Approved & Added', '✅')}
    <p style="color:#555;font-size:14px;margin:0 0 20px 0;"><strong>${approverName}</strong> approved a discovered lead and added it to the active pipeline.</p>

    <div style="background:#f0fdf4;border-left:4px solid #22c55e;border-radius:0 8px 8px 0;padding:20px;margin-bottom:24px;">
      <div style="font-size:18px;font-weight:700;color:${INK};margin-bottom:4px;">${lead.org_name}</div>
      <div style="margin-bottom:12px;">${badge(lead.type === 'school' ? '🏫 School' : '🏢 Corporate', '#22c55e')} ${badge(lead.city || '', TEAL)}</div>
      <table cellpadding="0" cellspacing="0">
        ${infoRow('Contact', lead.contact_name)}
        ${infoRow('Email', lead.email)}
        ${infoRow('Phone', lead.phone)}
        ${infoRow('Est. Value', formatINR(lead.contract_value))}
        ${infoRow('AI Score', lead.ai_score ? `${lead.ai_score}/100` : null)}
        ${infoRow('Stage', lead.stage)}
      </table>
    </div>
  ...
  await sendEmail({
    to: TEAM,
    subject: `✅ Lead Approved: ${lead.org_name} — Added to Pipeline`,
    html,
  });
}
...
async function sendFollowupScheduledEmail(followup, lead) {
  ...
  const html = baseTemplate(
    ...
    <div style="background:#f5f3f9;border-radius:10px;padding:20px;margin-bottom:24px;">
      ...
      <div style="margin:8px 0;">${badge(followup.channel?.toUpperCase())} ${badge(followup.owner === 'S' ? 'Sairam' : 'Abhijay', TEAL)}</div>
    ...
    const rows = items.map((f) => {
      ...
      <tr>
        ...
        <td style="padding:8px 0;border-bottom:1px solid #ede8f4;text-align:right;vertical-align:top;">
          ...
          <br><span style="font-size:11px;color:#999;">${f.owner === 'S' ? 'Sairam' : 'Abhijay'}</span>
        </td>
      </tr>`;
    }).join('');
    return `
  ...
  const rows = items.slice(0, 8).map((f) => {
    ...
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #ede8f4;">
        ...
        <div style="font-size:12px;color:#888;margin-top:2px;">${channelEmoji} ${f.action} · ${f.owner === 'S' ? 'Sairam' : 'Abhijay'}</div>
      </td>
      <td style="padding:10px 0;border-bottom:1px solid #ede8f4;text-align:right;vertical-align:top;">
