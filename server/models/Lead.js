...
const leadSchema = new mongoose.Schema({
  ...
  role:                  { type: String, trim: true },       // actual contact title
  ...
  source_url:            { type: String, trim: true },       // website lead was found on
  discovery_query:       { type: String, trim: true },       // search query that found this lead
  ...
  discovery_source: { type: String, enum: ['google_search', 'linkedin', 'news', 'referral', null], default: null },
...
leadSchema.pre('save', function (next) { this.updated_at = new Date(); next(); });
leadSchema.pre('findOneAndUpdate', function (next) { this.set({ updated_at: new Date() }); next(); });
